...
This correction has a significant affect on the shape of the blackbody curve used to extract specific intensity from temperature of stellar surface elements. The following figure shows the lcurve "average light curve flux" output for a single star with $T=22,000~{\rm K}$ at 30 different wavelengths. These were generated by using $\texttt{lroche}$ to generate 30 model light curves of a nonvariable single-star at 30 different wavelengths, then calculating the mean flux of each. Red points show the old Subs::planck.cc output; green points show the corrected output. The red and green curves are the full analytic curves for these equations.

![Comparison between the incorrect and corrected Planck function outputs at different wavelength](figures/updated_planck_function.jpg)
